...
const API_BASE_URL = `${BACKEND_ORIGIN}/api/daily-equipment-status`;
let equipmentList = [];
let currentStatus = [];
let historyData = [];
...
function getStatusIcon(status) {
  switch (status) {
    ...
    case "standby":
      return "bi-pause-circle text-info";
    default:
...
function getStatusColorClass(status) {
  switch (status) {
    ...
    case "breakdown":
      return "border-start border-2 border-danger";
    case "maintenance":
...
function updateStats(statuses) {
  ...
  const readyCount = statuses.filter(
    (s) => s.current_status === "ready"
  ).length;
  const breakdownCount = statuses.filter(
    (s) => s.current_status === "breakdown"
  ).length;
  const maintenanceCount = statuses.filter(
    (s) => s.current_status === "maintenance"
  ).length;
  ...
  if (alertsList) {
    if (alertCount > 0) {
      ...
      statuses
        .filter(
          (s) =>
            s.current_status === "breakdown" ||
            s.current_status === "maintenance"
        )
        .forEach((equipment) => {
          const statusText =
            equipment.current_status === "breakdown"
              ? "Breakdown"
              : "Maintenance";
          alertItems += `
                        <li><a class="dropdown-item text-${
                          equipment.current_status === "breakdown"
                            ...
                            <i class="bi ${
                              getStatusIcon(equipment.current_status).split(
                                " "
                              )[1]
                            } me-2"></i>
      ...
      alertsList.innerHTML =
        '<li><a class="dropdown-item" href="#">No alerts</a></li>';
    }
  }
}
            ...
            ${statuses
              .map(
                ...
                <div class="col-xl-3 col-lg-4 col-md-6">
                    <div class="card equipment-card h-100 ${getStatusColorClass(
                      equipment.current_status
                    )}" 
                         onclick="viewEquipmentHistory(${
                        ...
                        <div class="card-body">
                            ...
                            <div class="mt-3">
                                <div class="d-flex justify-content-between align-items-center mb-1">
                                    ...
                                    <span class="badge ${
                                      equipment.current_status === "ready"
                                        ? "bg-success"
                                        : equipment.current_status ===
                                          "breakdown"
                                        ? "bg-danger"
                            ...
                            <div class="mt-3">
                                <button class="btn btn-sm btn-outline-primary w-100" 
                                        onclick="event.stopPropagation(); quickUpdateEquipment(${
                                          equipment.equipment_id
                                        })">
...
function quickUpdateEquipment(equipmentId) {
  ...
  const modal = new bootstrap.Modal(
    document.getElementById("quickUpdateModal")
  );
  modal.show();
}
...
async function viewEquipmentHistory(equipmentId) {
  try {
    ...
    const equipmentName = equipment
      ? `${equipment.unit_code} - ${equipment.equipment_type}`
    ...
    .map(
      (record) => `
        <tr>
            ...
            <td>
                <span class="badge ${
                  record.equipment_status === "ready"
                    ? "bg-success"
                    : record.equipment_status === "breakdown"
                    ? "bg-danger"
                    : record.equipment_status === "maintenance"
                    ? "bg-warning"
                    : "bg-info"
                }">
                    ${record.equipment_status}
    ...
    if (container) {
      container.innerHTML = `
                <div class="text-center py-4">
                    ...
                    <p class="mt-2 text-muted">Loading status for ${formatDate(
                      date
                    )}...</p>
                </div>
            `;
...
function renderDateStatus(statuses, date) {
  ...
  container.innerHTML = `
        <div class="alert alert-info">
            ...
            Showing equipment status as of <strong>${formatDate(
              date
            )}</strong>. 
                    ...
                    ${statuses
                      .map(
                        ...
                        <tr>
                            ...
                            <td>
                                <span class="badge ${
                                  equipment.status_on_date === "ready"
                                    ? "bg-success"
                                    : equipment.status_on_date === "breakdown"
                                    ? "bg-danger"
                                    : equipment.status_on_date === "maintenance"
                                    ? "bg-warning"
                                    : "bg-info"
                                }">
...
async function deleteStatusRecord(statusId) {
  if (
    !confirm(
      "Are you sure you want to delete this status record? This will remove it from history but won't affect current status."
    )
  ) {
    return;
  }
